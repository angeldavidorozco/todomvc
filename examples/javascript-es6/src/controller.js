class Controller {
	...
	removeCompletedItems() {
		this.model.read({ completed: true }, (data) => {
			for (const item of data) this.removeItem(item.id);
		});
	...
	toggleAll(completed) {
		this.model.read({ completed: !completed }, (data) => {
			for (const item of data) this.toggleComplete(item.id, completed, true);
		});
	...
	_updateFilter(currentPage) {
		this._activeRoute = currentPage === "" ? "All" : currentPage;
		this._filter();
